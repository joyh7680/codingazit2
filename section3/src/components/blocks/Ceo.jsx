...
export default function Ceo({ className }) {
  const css = {
    ...
    ceo: tw`w-6/10 pt-40 pr-20 pl-40 text-right max-xl:pl-0 max-lg:pt-0 max-sm:w-full max-sm:pr-0 max-sm:text-left`,
    pic: tw`h-[37vmax] w-4/10 max-sm:h-[60vh] max-sm:w-full`
  };

  return (
    <section className={twMerge(css.sec, className)}>
      <InfoBox title={ceoInfo.title} content={ceoInfo.content} className={css.ceo} />
