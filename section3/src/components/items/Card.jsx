...
export default function Card({
  ...
  const css = {
    ...
    pic: tw`mb-6 block h-[12vmax] w-full max-lg:h-[17vmin] max-md:h-[36vmin]`,
  ...
  return (
    <Tag className={twMerge(css.div, className)}>
      {imgSrc && <Pic src={imgSrc} h_auto={false} url={url} shadow={thumbShadow} className={css.pic} />}

      {title && (
        <h1 className={css.h1}>
          <Link to={url}>{title.length > 40 ? title.slice(0, 40) + "..." : title}</Link>
        </h1>
      )}

      {content && <p className={css.p}>{content.length > 200 ? content.slice(0, 200) + "..." : content}</p>}
